...
#elif defined MSWIN					//for MS-Win only
void UserInterface::sethInstance(HANDLE h)
{
    my_hInstance=h;    //setters
}
void UserInterface::sethPrevInstance(HANDLE h)
{
    my_hPrevInstance=h;
}
void UserInterface::setlpCmdLine(LPSTR l)
{
    my_lpCmdLine=l;
}
void UserInterface::setnCmdShow(int i)
{
    my_nCmdShow=i;
}
HANDLE UserInterface::gethInstance(void)
{
    return my_hInstance;    //getters
}
HANDLE UserInterface::gethPrevInstance(void)
{
    return my_hPrevInstance;
}
LPSTR UserInterface::getlpCmdLine(void)
{
    return my_lpCmdLine;
}
int UserInterface::getnCmdShow(void)
{
    return my_nCmdShow;
}
HWND UserInterface::gethwnd(void)
{
    return my_hwnd;
}
HDC UserInterface::gethdcmem(void)
{
    return hdcMemory;
}
...
direction UserInterface::stick(void)  	//returns the inputted direction
{
    if (in==Mouse)  			//if input is from mouse
    {
        ...
        x=p.x;
        y=p.y;				//set current coordinates to these
        if (x==last_x && y==last_y) return none;//if not moved then none
        int dx=x-last_x;
        int dy=y-last_y;	//get changes of coordinates
        if (dx>=dy && dx>-dy) return right;	//destine directions
...
{
    x=xx;
    y=yy;			//setting current mouse coordinates and telling
    SetFocus(my_hwnd);		//MS-Win to take input from our window
}

void UserInterface::setquit(void)
{
    quit=1;    //set quit
}
...
static long FAR PASCAL MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY:    		// message: destroy from window manager
    {
        ui->setquit();		//set quit
        PostQuitMessage(0);
    }	//post a message that says we quit
    break;
    default:
